...
from mannequin.basicnet import Affine, LReLU, Input
...
def run():
    ...
    for i in range(10000):
        ...
        _, bckprp = decoded.evaluate(representation, sample=pics)
        decOptimizer.apply_gradient(bckprp(1.))
        latentGrad = latent.last_gradient

        _, bckprp = dkl.evaluate(pics)
        dklGrad = bckprp(-1.)
        ...
        if i % 100 == 99:
            plt.imshow(changedPics[43].reshape(28,28), cmap="gray")
            plt.show()
            plt.imshow(pics[43].reshape(28,28), cmap="gray")
            plt.show()
